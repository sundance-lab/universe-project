...
function hexToRgb(hex) {
    if (!hex || typeof hex !== 'string' || hex.length < 7 || hex[0] !== '#') { // Changed length to 7 to include '#'
...
self.onmessage = function(e) {
    const { cmd, planetData, rotationQuaternion, canvasWidth, canvasHeight, senderId, planetRadiusOverride } = e.data; // Added planetRadiusOverride
    ...
    if (cmd === 'renderPlanet') {
        ...
        const planetRadius = planetRadiusOverride || Math.min(canvasWidth, canvasHeight) / 2 * 0.9; // Use override if provided
