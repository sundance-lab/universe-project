...
const NUM_DUST_PARTICLES = 60000; // Increased dust particles
const NUM_ARM_PARTICLES = 250000; // Significantly increased arm particles
const NUM_ARMS = 6;
const BULGE_PARTICLES = 30000; // Slightly increased bulge particles
...
function _initScene(canvas, galaxy) {
    ...
    camera.position.set(0, GALAXY_RADIUS / 2, GALAXY_RADIUS * 2.5); // Slightly further back
    ...
    controls.maxDistance = GALAXY_RADIUS * 12; // Further zoom out
...
function _createGalacticBulge() {
    const positions = [];
    const colors = [];
    const color = new THREE.Color(&#39;#ffe0a3&#39;); // Brighter, more central yellow
    ...
    const material = new THREE.PointsMaterial({
        size: 2.5, // Slightly larger bulge particles
...
function _createGalaxyArms() {
    const positions = [];
    const colors = [];
    const colorInside = new THREE.Color(&#39;#fffacd&#39;); // Off-white/pale yellow for arms
    const colorOutside = new THREE.Color(&#39;#add8e6&#39;); // Light blue for outer arms

    for (let i = 0; i &lt; NUM_ARM_PARTICLES; i++) {
        const armIndex = i % NUM_ARMS;
        const startAngleOffset = [0, 1.1, 2.1, 3.2, 4.2, 5.3][armIndex % NUM_ARMS]; // Slightly adjusted offsets
        const angle = (i / (NUM_ARM_PARTICLES / NUM_ARMS)) * Math.PI * (4 + Math.random() * 0.8); // Tighter spiral
        const armRotation = (armIndex / NUM_ARMS) * Math.PI * 2 + startAngleOffset;
        const distance = Math.pow(i / (NUM_ARM_PARTICLES / NUM_ARMS), 0.65 + Math.random() * 0.15) * GALAXY_RADIUS; // More concentrated inner parts

        const randomX = (Math.random() - 0.5) * 80 * Math.pow(1 - (distance / GALAXY_RADIUS), 2); // Less spread at outer edges
    ...
    geometry.setAttribute(&#39;color&#39;, new THREE.Float32BufferAttribute(colors, 3));
    const material = new THREE.PointsMaterial({
        size: 1.2, // Slightly smaller arm particles
        sizeAttenuation: true,
        depthWrite: false,
        blending: THREE.AdditiveBlending,
        vertexColors: true
...
function _createDustLanes() {
    ...
    for (let i = 0; i &lt; NUM_DUST_PARTICLES; i++) {
        const armIndex = i % NUM_ARMS;
        const startAngleOffset = [0.1, 1.2, 2.2, 3.3, 4.3, 5.4][armIndex % NUM_ARMS]; // Offset dust more precisely
        const angle = (i / (NUM_DUST_PARTICLES / NUM_ARMS)) * Math.PI * (4 + Math.random() * 0.8);
        const armRotation = (armIndex / NUM_ARMS) * Math.PI * 2 + startAngleOffset;
        const distance = Math.pow(i / (NUM_DUST_PARTICLES / NUM_ARMS), 0.7 + Math.random() * 0.1) * GALAXY_RADIUS * 0.95; // Dust extends slightly further

        const randomX = (Math.random() - 0.5) * 90 * Math.pow(1 - (distance / GALAXY_RADIUS), 1.5); // Tighter dust lanes
...
function _animate() {
    ...
    galaxyGroup.rotation.y += 0.00015; // Slightly slower rotation
    controls.update();
