...
export class SunRenderer {
  ...
  #createSun = () => {
   ...
   // In the #createSun method, update the sunMaterial definition:
const sunMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    // Add new uniforms for 3D effect
  ...
  fragmentShader: `
    ...
    // Keep existing Simplex noise function
    ...
    void main() {
      // Calculate spherical shading
      vec2 center = vec2(0.5, 0.5);
      float dist = length(vUv - center);
      float sphericalFalloff = mix(centerBrightness, edgeDarkness, pow(dist * 2.0, 2.0));
      
      // Generate normal perturbation for 3D effect
      float bump = snoise(vec3(vUv * noiseScale * 2.0, time * 0.1)) * bumpStrength;
      vec3 normal = normalize(vec3(
        snoise(vec3(vUv * noiseScale * 2.0 + vec2(0.01, 0.0), time * 0.1)) - bump,
        snoise(vec3(vUv * noiseScale * 2.0 + vec2(0.0, 0.01), time * 0.1)) - bump,
        1.0
      ));
      ...
      // Combine existing noise effect with new 3D effects
      ...
      // Apply all effects
      finalColor *= pulse * sphericalFalloff * (diffuse * 0.3 + 0.7);
      
      // Keep existing alpha calculation
    ...
    const coronaMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            // Enhanced corona glow
            float alpha = smoothstep(0.5, 0.2, dist) * 0.6;
            // Animate corona intensity slightly
            alpha *= 0.8 + sin(time * 0.5) * 0.2;
