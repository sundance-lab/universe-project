...
function quat_identity() { return [1,0,0,0]; }
  function quat_from_axis_angle(axis, angle) { const hA = angle0.5, s=Math.sin(hA); return [Math.cos(hA),axis[0]s,axis[1]s,axis[2]s];}
  function quat_multiply(q1, q2) { const w1=q1[0],x1=q1[1],y1=q1[2],z1=q1[3],w2=q2[0],x2=q2[1],y2=q2[2],z2=q2[3]; return [w1w2-x1x2-y1y2-z1z2,w1x2+x1w2+y1z2-z1y2,w1y2-x1z2+y1w2+z1x2,w1z2+x1y2-y1x2+z1w2];}
...
function checkOverlap(r1, r2) { return!(r1.x+r1.width<r2.x||r2.x+r2.width<r1.x||r1.y+r1.height<r2.y||r2.y+r2.height<r1.y)}
  function getNonOverlappingPositionInCircle(pr, od, exR) { let plr=pr-(od/2)-5;if(plr<0)plr=0;for(let i=0;i<MAX_PLACEMENT_ATTEMPTS;i++){const a=Math.random()2Math.PI,r=Math.sqrt(Math.random())plr,cx=pr+rMath.cos(a),cy=pr+rMath.sin(a),x=cx-(od/2),y=cy-(od/2),nr={x,y,width:od,height:od};if(!exR.some(er=>checkOverlap(nr,er)))return{x,y}}return null}
  ...
  function generateUniverseLayout() { const s=Math.min(window.innerWidth,window.innerHeight);gameSessionData.universe.diameter=Math.max(300,s0.85);if(universeCircle){universeCircle.style.width=${gameSessionData.universe.diameter}px;universeCircle.style.height=${gameSessionData.universe.diameter}px;universeCircle.style.backgroundColor=FIXED_COLORS.universeBg;}}
...
const initialOrbitalAngle=Math.random()2Math.PI,orbitalSpeed=Math.random()*(MAX_ROTATION_SPEED_RAD_PER_PERLIN_UNIT-MIN_ROTATION_SPEED_RAD_PER_PERLIN_UNIT)+MIN_ROTATION_SPEED_RAD_PER_PERLIN_UNIT;
    const initialAxialAngle=Math.random()2Math.PI,axialSpeed=DEFAULT_PLANET_AXIAL_SPEED;
...
function animateSolarSystem(now) {
   ...
   if(aSV && solarSystemScreen && solarSystemScreen.classList.contains('active') && aSV.planets){
    aSV.planets.forEach(p=>{
     if (p.element) {
      p.currentOrbitalAngle+=p.orbitalSpeed6dT;
      p.currentAxialAngle+=p.axialSpeed60dT;
      const pMOX=p.distanceMath.cos(p.currentOrbitalAngle);
      const pMOY=p.distanceMath.sin(p.currentOrbitalAngle);
...
window.addEventListener('mousemove', (e) => {
   ...
   if (isDraggingPlanetVisual && currentPlanetDisplayedInPanel && planetVisualCanvas && planetVisualPanel && planetVisualPanel.classList.contains('visible')) {
    ...
    const rAX=(dMY/cH)Math.PIPLANET_ROTATION_SENSITIVITY,rAY=(dMX/cW)(2Math.PI)PLANET_ROTATION_SENSITIVITY;
    ...
    const rAX=(dMY/cBFH)Math.PIPLANET_ROTATION_SENSITIVITY,rAY=(dMX/cBFW)(2*Math.PI)*PLANET_ROTATION_SENSITIVITY;
    const iXQ=quat_from_axis_angle([1,0,0],-rAX),iYQ=quat_from_axis_angle([0,1,0],rAY);
    const cIQ=quat_multiply(iYQ,iXQ);
