document.addEventListener('DOMContentLoaded', () => {
  ...
  function setActiveScreen(screenToShow) {
    ...
    if (screenToShow === planetDesignerScreen || (customizationModal && customizationModal.classList.contains('visible'))) { // Check modal visibility too
        ...
        if (bestConnection) {
          ...
          if (isValidPotentialConnection) {
            ...
            if (forcedTargetId) {
              ...
              for(const connectedId of connectedSystems) {
                const connectedSystem = allSystemsCalc.find(s => s.id === connectedId);
                if (!connectedSystem) continue; // Should not happen if IDs are consistent
                const targetSystem = allSystemsCalc.find(s => s.id === targetUnconnectedId);
                if (!targetSystem) continue; // Should not happen
              ...
              if(ultimateForceTargetId) {
                ...
                // If a system literally cannot connect, remove it.
  ...
  function switchToSolarSystemView(solarSystemId) {
    ...
    currentSunSize = Math.max(currentSunSize, 15); // Ensure sun is not too small
    ...
    for (let i = 0; i < numPlanets; i++) {
      ...
      if (Math.random() < 0.5) {
        ...
        // Prepend the new gradient to the existing style (which includes the water color base)
  ...
  function animateSolarSystem(now) {
    ...
    if (activeSolarSystemView && solarSystemScreen && solarSystemScreen.classList.contains('active') && activeSolarSystemView.planets) {
      activeSolarSystemView.planets.forEach(p => {
        if (p.element) {
          p.currentOrbitalAngle += p.orbitalSpeed * 6 * deltaTime;
          p.currentAxialAngle += p.axialSpeed * 60 * deltaTime;
  ...
  function handleZoom(direction, mouseEvent = null) {
    ...
    if (Math.abs(oldZoom - newCurrentZoom) < 0.0001) return;
    ...
    if (isSolarSystemView && viewportElement) {
      ...
      if (currentShowPlanetOrbits) drawAllOrbits();
      renderFunction(true);
      startSolarSystemAnimation();
    } else {
  ...
  function panMouseMove(event) {
    ...
    if (panningState.viewportElement === galaxyViewport && panningState.dataObject) {
        clampGalaxyPan(panningState.dataObject);
        renderGalaxyDetailScreen(true);
    } else if (panningState.viewportElement === solarSystemScreen && panningState.viewportElement && panningState.dataObject) {
  ...
  function panMouseUp() {
    ...
    if (panningState.targetElement) panningState.targetElement.style.transition = '';
  ...
  function regenerateCurrentUniverseState(force = false) {
    ...
    // gameSessionData.customPlanetDesigns = []; // Preserved by default
  ...
  if (applyCustomizationButton) applyCustomizationButton.addEventListener('click', () => {
    ...
    setActiveScreen(mainScreen); // Ensure main screen is active after regeneration
  });
  ...
  window.addEventListener('resize', () => {
    ...
    gameSessionData = {
      ...
      isInitialized: false,
      panning: { isActive: false, startX: 0, startY: 0, initialPanX: 0, initialPanY: 0, targetElement: null, viewportElement: null, dataObject: null },
      customPlanetDesigns: preservedCustomDesigns
    };
    ...
    if (screenToActivate) {
      if (currentScreenId === 'planet-designer-screen') {
        ...
        // Need to ensure activeGalaxyId might have changed if galaxies were regenerated by ID
        // For simplicity, we assume IDs might persist or we go to main.
        // A more robust solution would re-find the galaxy by some persistent property or regenerate selection.
        const stillExists = gameSessionData.galaxies.find(g => g.id === gameSessionData.activeGalaxyId)
        if(stillExists) switchToGalaxyDetailView(gameSessionData.activeGalaxyId);
        else switchToMainView();
      } else if (currentScreenId === 'solar-system-screen' && gameSessionData.activeSolarSystemId) {
         // Similar to galaxy, ensure solar system context can be restored
        const galaxyPartMatch = gameSessionData.activeSolarSystemId.match(/^(galaxy-\d+)-ss-\d+$/);
        const galaxyPart = galaxyPartMatch ? galaxyPartMatch[1] : null;
        const activeGalaxy = galaxyPart ? gameSessionData.galaxies.find(g => g.id === galaxyPart) : null;
        if(activeGalaxy && activeGalaxy.solarSystems.find(ss => ss.id === gameSessionData.activeSolarSystemId)){
  ...
  if (backToGalaxyButton) backToGalaxyButton.addEventListener('click', () => {
    if (gameSessionData.activeGalaxyId) {
  ...
  if (galaxyViewport) {
    ...
    galaxyViewport.addEventListener('mousedown', (e) => {
      ...
      if (gal) { // Ensure gal is found before trying to access its properties
          galaxyLastPanSpecific.x = gal.currentPanX || 0;
          galaxyLastPanSpecific.y = gal.currentPanY || 0;
      } else { // Fallback if galaxy isn't found, though this state should ideally not occur
          galaxyLastPanSpecific.x = 0;
          galaxyLastPanSpecific.y = 0;
      }
      ...
      gameSessionData.panning.isActive = true;
    });
    ...
    const galaxyMouseUpHandler = () => {
      if (isGalaxyPanningSpecific) {
        ...
        renderGalaxyDetailScreen(false);
        gameSessionData.panning.isActive = false;
      }
    };
