function generatePlanetInstanceFromBasis(basis, isForDesignerPreview = false) {
  ...
  const getValueFromRange = (range, defaultValue, defaultSpread = 1.0) => {
    ...
    // Fallback for older format or single values
    if (typeof range === 'number') return range;
    // If completely undefined, use default with a bit of spread
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- STATE VARIABLES ---
  let linesCtx;
  let solarSystemOrbitCanvasEl; // This is created dynamically in switchToSolarSystemView
  let orbitCtx;
  ...
  // let planetVisualRotationQuatTarget = quat_identity(); // Seems unused with current worker-based rotation
  // let planetVisualRotationQuatDisplayed = quat_identity(); // Seems unused
    ...
    try {
        ...
        planetVisualWorker.onmessage = function(e) {
          ...
          if (senderId === 'planet-visual-canvas' && planetVisualCanvas) {
            ...
            if (renderedData && width && height) {
              try {
                ...
                planetVisualCanvas.style.transform = ""; // Clear any CSS transform
                // planetVisualRotationQuatDisplayed = planetVisualRotationQuatTarget; // Seems unused
              } catch (err) {
          ...
          if (needsPlanetVisualRerender && currentPlanetDisplayedInPanel && planetVisualPanel.classList.contains('visible')) {
            ...
            // isRenderingVisualPlanet = true; // renderPlanetVisual will set this
          ...
          planetVisualWorker.onerror = function(error) {
            console.error("Error in planetVisualWorker:", error.message, error);
          };
        ...
        designerWorker.onerror = function(error) {
            console.error("Error in designerWorker:", error.message, error);
        };
...
// Removed duplicated resizeDesignerCanvasToDisplaySize function from here. It's defined at the top.
...
function updateBasisAndRefreshDesignerPreview() {
  ...
  if (minH_min > minH_max) [minH_min, minH_max] = [minH_max, minH_min]; // Swap if min > max
  ...
  // Ensure terrain hierarchy: minHeight < oceanHeight < maxHeight
  // This is a simplified validation, more complex inter-dependencies might be desired
  if (minH_max > oceanH_min) oceanH_min = minH_max + 0.1;
  if (oceanH_min > oceanH_max) oceanH_max = oceanH_min + 0.1; // Ensure ocean range is valid
  if (oceanH_max > maxH_min) maxH_min = oceanH_max + 0.1;
  if (maxH_min > maxH_max) maxH_max = maxH_min + 0.1; // Ensure max height range is valid
...
function generateAndRenderDesignerPreviewInstance(resetRotation = false) {
  currentDesignerPlanetInstance = generatePlanetInstanceFromBasis(currentDesignerBasis, true); // Pass true for designer
  if (resetRotation) designerPlanetRotationQuat = quat_identity();
  resizeDesignerCanvasToDisplaySize();
  if (!isRenderingDesignerPlanet && designerWorker) { // Check if worker exists
...
function randomizeDesignerPlanet() {
  ...
  currentDesignerBasis.continentSeed = Math.random(); // New seed for the basis
  ...
  function saveCustomPlanetDesign() {
    ...
    const newDesign = {
      ...
      continentSeed: currentDesignerBasis.continentSeed, // Save the basis seed
  ...
  function loadAndPreviewDesign(designId) {
    ...
    if (designToLoad) {
      ...
      currentDesignerBasis.continentSeed = designToLoad.continentSeed || Math.random(); // Use saved seed for basis

      // Ensure ranges are arrays of 2 numbers, potentially migrating from older single values
      // The generatePlanetInstanceFromBasis also has fallbacks, but good to keep basis clean
      const ensureRange = (value, defaultVal, spread) => {
          if (Array.isArray(value) && value.length === 2 && typeof value[0] === 'number' && typeof value[1] === 'number') {
              return [...value];
          }
          const base = typeof value === 'number' ? value : (typeof defaultVal === 'number' ? defaultVal : 0);
          return [base, base + (typeof spread === 'number' ? spread : 1.0)];
      };
      
      currentDesignerBasis.minTerrainHeightRange = ensureRange(
        designToLoad.minTerrainHeightRange || designToLoad.minTerrainHeight, // Check old single prop too
  ...
  function switchToPlanetDesignerScreen() {
    ...
    populateDesignerInputsFromBasis();
    populateSavedDesignsList();
    resizeDesignerCanvasToDisplaySize(); // Call resize function directly
    requestAnimationFrame(() => { // Defer render to ensure canvas dimensions are updated
      generateAndRenderDesignerPreviewInstance(true);
    });
  }
...
if(designerCancelBtn) designerCancelBtn.addEventListener('click', () => setActiveScreen(mainScreen));
if(createPlanetDesignButton) createPlanetDesignButton.addEventListener('click', switchToPlanetDesignerScreen);
    ...
    try {
      ...
      if (savedStateString) {
        ...
        if (loadedState && typeof loadedState.universeDiameter === 'number' && Array.isArray(loadedState.galaxies)) {
          ...
          // Migrate customPlanetDesigns to ensure they have range properties
          gameSessionData.customPlanetDesigns = (loadedState.customPlanetDesigns || []).map(design => {
            const migratedDesign = { ...design };
            // Ensure continentSeed exists, important for basis consistency in designer
            ...
            const ensureRange = (value, oldSingleProp, defaultVal, spread) => {
                if (Array.isArray(value) && value.length === 2 && typeof value[0] === 'number' && typeof value[1] === 'number') {
                    return [...value]; // Already a valid range
                }
                // Try to use old single property if range is invalid/missing
            ...
            // Clean up old single properties after migration
  ...
  function renderGalaxyDetailScreen(isInteractive = false) {
    ...
    g.solarSystems.forEach(ss=>{
      ...
      el.style.width=`${dSIP}px`; 
      el.style.height=`${dSIP}px`;
      const cO=dSIP/2;
  ...
  function renderSolarSystemScreen(isInteractive = false) {
    ...
    const gP=gameSessionData.activeSolarSystemId.substring(0,gameSessionData.activeSolarSystemId.indexOf('-ss-'));
    const aG=gameSessionData.galaxies.find(g=>g.id===gP);
    let sSO=null;
    if(aG&&aG.solarSystems){sSO=aG.solarSystems.find(ss=>ss.id===d.systemId);}
  ...
  function switchToGalaxyDetailView(galaxyId) {
    ...
    makeTitleEditable(galaxyDetailTitleText,galaxyDetailTitleInput,(nN)=>{g.customName=nN||null;saveGameState();renderMainScreen();return g.customName||`Galaxy ${dId}`;});
  ...
  function renderPlanetVisual(planetData, rotationQuaternion, targetCanvas = planetVisualCanvas) {
    if (!planetData || !targetCanvas || (!planetVisualWorker && targetCanvas === planetVisualCanvas) || (!designerWorker && targetCanvas === designerPlanetCanvas)) {
        console.warn("renderPlanetVisual: Missing data, canvas, or appropriate worker.", { planetData, targetCanvas, workersExist: !!(planetVisualWorker || designerWorker) });
    ...
    if (!pD.continentSeed && pD.continentSeed !== 0) pD.continentSeed = Math.random(); // continentSeed can be 0
  ...
  function switchToSolarSystemView(solarSystemId) {
    ...
    for (let i = 0; i < numPlanets; i++) {
      ...
      const basisToUse = (gameSessionData.customPlanetDesigns.length > 0) 
        ...
        : { // Default basis if no custom designs exist
            ...
            // continentSeed is handled by generatePlanetInstanceFromBasis(..., false)
            minTerrainHeightRange: [0.0, 1.0], 
            maxTerrainHeightRange: [5.0, 8.0], 
            oceanHeightRange: [1.0, 3.0] 
          };
      
      const newPlanetData = generatePlanetInstanceFromBasis(basisToUse, false); // false for game instances
      ...
      planetEl.addEventListener('click',(e)=>{
          ...
          if(planetVisualTitle) planetVisualTitle.textContent=newPlanet.planetName;
          if(planetVisualSize) planetVisualSize.textContent=Math.round(newPlanet.size);
          if(planetVisualPanel) planetVisualPanel.classList.add('visible');
          if(!wasPanelVisible && planetVisualPanel){
          ...
          } else if (planetVisualPanel) {
              planetVisualPanel.style.transition='none';
          }
          planetVisualRotationQuat=quat_identity();
          // isRenderingVisualPlanet will be set by renderPlanetVisual if it proceeds
    ...
    makeTitleEditable(solarSystemTitleText,solarSystemTitleInput,(nN)=>{
        if(solarSystemObject){
            ...
            saveGameState();
            renderGalaxyDetailScreen(); // Re-render galaxy screen in case name changed there too
  ...
  function animateSolarSystem(now) {
    ...
    if(aSV && solarSystemScreen && solarSystemScreen.classList.contains('active') && aSV.planets){
      aSV.planets.forEach(p=>{
        if (p.element) { // Ensure element exists
          ...
          p.element.style.left=`calc(50% + ${pMOX}px)`;
          p.element.style.top=`calc(50% + ${pMOY}px)`;
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    if (isDraggingPlanetVisual && currentPlanetDisplayedInPanel && planetVisualCanvas && planetVisualPanel.classList.contains('visible')) {
      ...
      if (!isRenderingVisualPlanet && planetVisualWorker) { 
        // isRenderingVisualPlanet = true; // Set by renderPlanetVisual
        renderPlanetVisual(currentPlanetDisplayedInPanel, planetVisualRotationQuat, planetVisualCanvas); 
      } else {
      ...
      const cBFW=(designerPlanetCanvas.width > 0 ? designerPlanetCanvas.width : rect.width) || 1; // ensure not zero
      const cBFH=(designerPlanetCanvas.height > 0 ? designerPlanetCanvas.height : rect.height) || 1; // ensure not zero
      ...
      if (!isRenderingDesignerPlanet && currentDesignerPlanetInstance && designerWorker) { 
        // isRenderingDesignerPlanet = true; // Set by renderPlanetVisual (called by renderDesignerPlanet)
        renderDesignerPlanet(currentDesignerPlanetInstance, designerPlanetRotationQuat);
      }
    }
  });

  // function applyPlanetPanelVisualRotation() { // This function seems unused with worker-based rendering
  //   const q=planetVisualRotationQuatTarget; // Target is also unused mostly
  //   const yS=q[2]*q[2];let t0=+2.0*(q[0]*q[1]+q[2]*q[3]);let t1=+1.0-2.0*(q[1]*q[1]+yS);let rX=Math.atan2(t0,t1);let t2=+2.0*(q[0]*q[2]-q[3]*q[1]);t2=t2>1?1:t2;t2=t2<-1?-1:t2;let pY=Math.asin(t2);
  //   if (planetVisualCanvas) planetVisualCanvas.style.transform=`rotateX(${pY*180/Math.PI}deg) rotateY(${rX*180/Math.PI}deg)`;
  // }
  ...
  window.addEventListener('resize', () => {
    ...
    // Clear almost all state - drastic, but matches original logic for resize.
    // Consider selectively updating layouts instead if this causes undesirable full resets.
    ...
    if(galaxyZoomContent){
        ...
        if(canvasLines) galaxyZoomContent.appendChild(canvasLines); // Re-add canvas if it exists
    }
    if(solarSystemContent) solarSystemContent.innerHTML='';
    ...
    loadCustomizationSettings(); // Reload settings
    initializeGame(true); // Force regeneration with new window size
    
    const sTA = cSBRI ? document.getElementById(cSBRI) : mainScreen;
    setActiveScreen(sTA || mainScreen);
    
    if (sTA === planetDesignerScreen) {
      switchToPlanetDesignerScreen(); // Re-initialize designer screen
    ...
    } else if (sTA === mainScreen) {
        // renderMainScreen() is called by initializeGame
    }
    // Ensure canvases in active views are resized if necessary
    if (planetVisualPanel && planetVisualPanel.classList.contains('visible') && currentPlanetDisplayedInPanel) {
        // Consider resizing planetVisualCanvas if its size is dependent on panel or screen
        // For now, assume its fixed size or aspect ratio handles it. 
        // Re-render if needed.
        renderPlanetVisual(currentPlanetDisplayedInPanel, planetVisualRotationQuat, planetVisualCanvas);
    }
  });
  ...
  if (galaxyViewport) { 
    galaxyViewport.addEventListener('click', function (event) {
      // Prevent click-through if panning gesture was short (browser might still fire click)
      if (Math.abs(event.clientX - gameSessionData.panning.startX) > 5 || Math.abs(event.clientY - gameSessionData.panning.startY) > 5) {
          if (gameSessionData.panning.isActive) return; // If a true pan was intended
      }
    ...
    galaxyViewport.addEventListener('mousedown', (e) => {
      ...
      // if (gameSessionData.panning.isActive) return; // This check might be redundant with startPan's own checks
      ...
      // Integrate with general panning system if possible, or ensure they don't conflict
      // For now, assuming this is a separate pan handler for galaxyViewport specifically
      gameSessionData.panning.startX = e.clientX; // For click distance check above
    ...
    const galaxyMouseUpHandler = () => { // Removed 'e' as it's not used
